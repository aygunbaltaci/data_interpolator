...
# 
# It interpolates the data with high resolution  
# to the data with low resolution based on the input  
...
# this program to interpolate the data from GPS module 
# (data with high resolution) to the data from flight 
# controller (data with low resolution). This way, you
...
outputFile = '_datainterpolation_lowresolution_output'
...
def lookupNearest(x0): # taken from https://stackoverflow.com/questions/31734471/2d-nearest-neighbor-interpolation-in-python
    xi = np.abs(data_highresolution[0] - x0).argmin() # xi is the row number of the closest value in data_highresolution[0] to x0 
    return xi
...
with open(inputDir + os.sep + lowResInputFile, 'r', encoding = defaultEncoding) as csvfile:
    ...
    # Convert input data_lowresolution to float  
...
with open(inputDir + os.sep + highResInputFile,'r', encoding = defaultEncoding) as csvfile:
    ...
    if not (data_highresolution[0][0].isdigit()): # only check one of the first-row entries. If one of them is not a number, then the other first-row entries should be the same        
        for i in range(len(data_highresolution)): # Delete labels
            headerRow.append(data_highresolution[i][0])
...
for i in range(0, len(data_lowresolution[0])): # 0 is the col number in data_lowresolution file
    value = lookupNearest(data_lowresolution[0][i]) # find the row for nearest point interpolation
    for j in range(len(data_lowresolution)):
        concatRow.append(data_lowresolution[j][i])
    for j in range(len(data_highresolution)):
        concatRow.append(data_highresolution[j][value])
